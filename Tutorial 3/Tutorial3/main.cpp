...
int main()
{
    ...
    while(window.isOpen())
    {
        ...
        //(Google translate) Handling keystrokes outside the event queue processing loop
        ...
        //(Google translate) Handling mouse clicks outside the event processing queue
        ...
        //(Google translate) Move the character with the mouse
        ...
        std::cout << "player position x = " << playerPosition.x
            << " y = " << playerPosition.y << std::endl;
        ...
        //(Google translate) For the same speed of character animation on computers
        //of different power the laptop performs the game cycle every 16 milliseconds
        frameCounter = updateFrame ? frameCounter +
            frameSpeed * clock.restart().asSeconds() : 0;
        ...
        pSprite.setTextureRect(
            sf::IntRect(
                source.x * pSize.x, source.y * pSize.y, pSize.x, pSize.y));
